...
function AsterixDBConnection(configuration) {
	...
	// By default, we will wait for calls to the REST API to complete. The query
	// method
	// sends a different setting when executed asynchronously. Calls that do not
	// specify a mode
	// will be executed synchronously.
	this._properties["mode"] = "synchronous";

	// These are the default error behaviors for Asterix and ajax errors,
	// respectively.
	// They can be overridden by calling initializing your AsterixDBConnection
	// like so:
	// adb = new AsterixDBConnection({
	// "error" : function(data) {
	// // override here...
	// });
	// and similarly for ajax_error, just pass in the configuration as a json
	// object.
	...
	// This is the default path to the local Asterix REST API. Can be
	// overwritten for remote configurations
	// or for demo setup purposes (such as with a proxy handler with Python or
	// PHP.
	this._properties["endpoint_root"] = "http://localhost:19002/";

	// If we have passed in a configuration, we will update the internal
	// properties
	// using that configuration. You can do things such as include a new
	// endpoint_root,
	// a new error function, a new dataverse, etc. You can even store extra
	// info.
	//
 ...
 * 
 * @param statements,
 *            statements of an AQL query
 * @param successFn,
 *            a function to execute if this query is run successfully
 * @param mode,
 *            a string either "synchronous" or "asynchronous", depending on
...
/**
 * query_status
 * (http://asterix.ics.uci.edu/documentation/api.html#QueryStatusApi)
 * 
 * @param handle,
 *            a json object of the form {"handle" : handleObject}, where the
 *            handle object is an opaque handle previously returned from an
 *            asynchronous call.
 * @param successFn,
 *            a function to call on successful execution of this API call.
 */
AsterixDBConnection.prototype.query_status = function(handle, successFn) {
	this._api(handle, successFn, "query/status");
...
/**
 * query_result
 * (http://asterix.ics.uci.edu/documentation/api.html#AsynchronousResultApi)
 * 
 * handle, a json object of the form {"handle" : handleObject}, where the handle
 * object is an opaque handle previously returned from an asynchronous call.
...
AsterixDBConnection.prototype.query_result = function(handle, successFn) {
	this._api(handle, successFn, "query/result");
 ...
 * 
 * @param statements,
 *            statements to run through ddl api
 * @param successFn,
 *            a function to execute if they are successful
 */
 ...
 * 
 * @param statements,
 *            statement(s) for an update API call
 * @param successFn,
 *            a function to run if this is executed successfully.
 * 
 * This is an AsterixDBConnection handler for the update API. It passes
 * statements provided to the internal API endpoint handler.
 */
 ...
 * 
 * @param statements,
 *            a string or a list of strings representing an Asterix query object
 * @param successFn,
 *            a function to execute if call succeeds
 * 
 * Queries without a dataverse. This is a work-around for an Asterix REST API
 * behavior that sometiems throws an error. This is handy for Asterix Metadata
 * queries.
 */
 ...
 * 
 * @param json,
 *            the data to be passed with the request
 * @param onSuccess,
 *            the success function to be run if this succeeds
 * @param endpoint,
 *            a string representing one of the Asterix API endpoints
 * 
...
AsterixDBConnection.prototype._api = function(json, onSuccess, endpoint) {
	...
	// The success function is called if the response is successful and returns
	// data,
	// or is just OK.
	var success_fn = onSuccess;

	// This is the error function. Called if something breaks either on the
	// Asterix side
	// or in the Ajax call.
	...
	// This SDK does not rely on jQuery, but utilizes its Ajax capabilities when
	// present.
	if (window.jQuery) {
		$.ajax({
			...
			// or reconfigured endpoint root along with the appropriate api
			// endpoint
			// such as "query" or "update".
			url : endpoint_url,

			// This is the data in the format specified on the API
			// documentation.
			data : json,

			// We send out the json datatype to make sure our data is parsed
			// correctly.
			dataType : "json",
			contentType : "application/x-www-form-urlencoded;charset=UTF-8",
			// The success option calls a function on success, which in this
			// case means
			// something was returned from the API. However, this does not mean
			// the call succeeded
			// on the REST API side, it just means we got something back. This
			// also contains the
			// error return codes, which need to be handled before we call th
			// success function.
			success : function(data) {

				// Check Asterix Response for errors
				// See
				// http://asterix.ics.uci.edu/documentation/api.html#ErrorCodes
				if (data["error-code"]) {
					error_fn(data);
			...
			// This is the function that gets called if there is an ajax-related
			// (non-Asterix)
			// error. Network errors, empty response bodies, syntax errors, and
			// a number of others
			// can pop up.
			error : function(data) {

				// Some of the Asterix API endpoints return empty responses on
				// success.
				// However, the ajax function treats these as errors while
				// reporting a
				// 200 OK code with no payload. So we will check for that,
				// otherwise
